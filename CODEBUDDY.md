...
- **新增能力**：`engine.js.simulateCMV` 新增可选参数 `rebalanceEveryMonths`（默认 1 = 每月检查一次，与现状零偏差）。
- **研究脚本**：
  - `scripts/rebalance_study.js` — 直接调用生产 `simulateCMV`，对 6 档频率（1/2/3/6/12/24 月）跑完全滚动 131 起点，输出 `_rebalance_study_report.md`。频率维度结论：年化波动 < 0.5pct，**当前「1 月 1 次 + ±5%」已是稳健型配置下接近最优**。
  - `scripts/fetch_daily.py` — 新浪日 K 线落盘（datalen=900 仅约 3 年，要做严格日频研究需扩展为分页拉取）。
- **关键发现**：
  - **频率维度**：1月/3月/6月/12月/24月频率的差异 **< 0.5 个百分点**，远小于"不同起点"差异（同一频率下不同起点年化差 4-7pct）。
  - **阈值维度**：±0.1% / ±2% / ±5% / ±10% / ±20% 阈值的差异 **< 0.5 个百分点**。
  - **意外洞察**："不调仓（建仓一次持有）" 全期年化 **12.21%**（最高），回撤 **-14.35%**（最大）；再平衡的本质是"放弃部分收益换更低回撤"，稳健型 25% 现金缓冲下，再平衡收益增量极小。
  - **入口时机**：2021-08 起点（约 5 年前入场）年化仅 +5.6%（受 2021 A 股高峰影响），2024-08 起点年化 +9.0% — **入场时机的影响是再平衡策略的 10 倍**。
...
- [ ] `scripts/fetch_daily.py` 当前 datalen=900 仅 ~3 年数据，若做严格"日级别再平衡研究"需扩展为分页拉取（按月循环）。项目数据精度为月的情况下日级别研究代价大，**优先级低**。
